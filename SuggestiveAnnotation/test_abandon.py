    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while step < num_iter and not coord.should_stop():
                ori_images, labels, logits, dice, _ = sess.run(metrics)
                avg_dice += dice
                step += 1
...
def evaluate():
    ...
    with tf.Graph().as_default() as g:
        ...
        images, labels = data_provider.input(eval_data=1, batch_size=1)
        ...
        dice_op = networks.dice_coef(logits, labels, loss_type='jaccard')
        ...
        # variable_averages = tf.train.ExponentialMovingAverage(networks.FLAGS.moving_average_decay, global_step)
        ...
        while True:
            eval_once(saver, [images, labels, logits, dice_op, test_op], summary_writer, summary_op)
            if FLAGS.run_once:
                break
