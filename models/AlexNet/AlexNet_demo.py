...
norm1 = normalization(conv1, "lrn1", depth_radius=4, bias=1.0, alpha=0.001/9, beta=0.75)
pool1 = max_pool(norm1, "pool1", ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')
print_activations(conv1)
print_activations(pool1)
parameters += params
conv2, params = convolution(pool1, "conv2", shape=[5, 5, 96, 256], sd=0.1)
norm2 = normalization(conv2, "lrn2", depth_radius=4, bias=1.0, alpha=0.001/9, beta=0.75)
pool2 = max_pool(norm2, "pool2", ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')
...
fname = "evaluation" + strftime("%m%d%H%M%S", localtime(time()))
